...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            ...
            <div class="d-flex flex-wrap justify-content-end mt-2">
                ...
                @can('alumni_import')
                    <button class="btn rounded-pill btn-info mt-1 me-1" type="button" onclick="alumniImport()">
                ...
                @can('alumni_trash')
                    <a href="{{ route('admin.alumni.trash') }}" class="btn rounded-pill btn-outline-danger mt-1 me-1">
            ...
            <!--Search Form -->
            <div class="card-body">
                <form id="search_alumni" action="" method="GET">
            ...
            <div class="card-datatable table-responsive">
                <table class="table border-top" id="base-table">
                    ...
                    <tbody>
                        @foreach ($alumnis as $key => $data)
                            <tr>
                                <td></td>
                                <td>{{ $key + 1 + ($alumnis->currentPage() - 1) * $alumnis->perPage() }}</td>
                                <td>{{ $data->name }}</td>
                ...
                <div class="d-flex justify-content-center m-3 text-muted">
                    @if (count($alumnis) > 0)
                        Showing {{ $alumnis->firstItem() }} to {{ $alumnis->lastItem() }} of
                        {{ $alumnis->total() }} entries
                    @endif
                </div>
    ...
    <script type="text/javascript">
        function create() {
            create_form_modal('/admin/alumni');
        }

        function alumniImport() {
            create_form_modal('/admin/alumni/alumniImport');
        }
        ...
        $(document).ready(function() {
            ...
            $("#base-table").on("click", ".view", function() {
                var id = $(this).attr('id');
                view_modal('/admin/alumni', id)
            });

            // Edit Form
            $("#base-table").on("click", ".edit", function() {
                var id = $(this).attr('id');
                edit_form_modal('/admin/alumni', id)
            });

            // Change Password Form
            $("#base-table").on("click", ".change_password", function() {
                var id = $(this).attr('id');
                edit_others_form_modal('/admin/alumni/password', id, 'Change Password')
            });

            // Delete
            $("#base-table").on("click", ".delete", function() {
                var id = $(this).attr('id');
                soft_delete('/admin/alumni', id)
            });
